...
class PipelineManager:
    ...
    def __init__(self,
        ...
        # Counter used to save the frames of video
        self.counter = 0

        # ----------------------------------------
        # CREATING OUTPUT PATHS: every step result(s) will be saved in a specific directory
        # ----------------------------------------
        self.paths_dict = {t: create_output_dir(self.output_base_path, MediaType.image, self.input_filename) for t in
                           Task}

        # Define output filename
        self.out_filename = ntpath.basename(self.input_filename)

    def save_image(self, img, task):
        """
        Save the given image. The destination depends on the task.
        """
        if self.media_type == MediaType.video:
            filename, ext = self.out_filename.split('.')
            out_filename = "_".join([filename, "{:05d}".format(self.counter), ext])
            self.counter += 1
        else:
            out_filename = self.out_filename
        out_path = os.path.join(self.paths_dict[task], out_filename)
    ...
    def __people_localization(self, paintings_detected):
        """
    ...
    def run(self, img_original):
        ...
        # ------------------------------------------------------------------------
        if self.task > Task.painting_detection and self.task != Task.people_detection:
            # ----------------------
            # PAINTING SEGMENTATION
            # ----------------------
        ...
        if self.task > Task.painting_segmentation and self.task != Task.people_detection:
            # ----------------------
            # PAINTING RECTIFICATION
            # ----------------------
        ...
        if self.task > Task.painting_rectification and self.task != Task.people_detection:
            # ----------------------
            # PAINTING RETRIEVAL
            # ----------------------
        ...
        if self.task > Task.painting_retrieval or self.task == Task.people_detection:
            # ----------------------
            # PEOPLE DETECTION
            # ----------------------
        ...
        if self.task > Task.people_detection:
            ...
            # PEOPLE LOCALIZATION
            # ----------------------
            people_room = self.__people_localization(paintings_detected)
